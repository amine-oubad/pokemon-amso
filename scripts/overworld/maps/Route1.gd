...
## Layout 320×240 : herbes hautes gauche/droite, chemin central.
...
func _build_decorations() -> void:
	# Herbes hautes (visuelles — sombre)
	...
	# Petits arbres (obstacles visuels décoratifs)
	...
	# Indicateur "← Bourg-Palette" en bas
	var lbl_s      := Label.new()
	...
	add_child(lbl_s)
	# Indicateur "Jadielle →" en haut
	var lbl_n      := Label.new()
	lbl_n.text      = "↑ Jadielle (bientôt)"
...
func _build_borders() -> void:
	...
	# Bas — deux segments, trou pour le chemin (x=144-176)
...
func _build_transitions() -> void:
	...
	_transition(Vector2(160.0, MAP_H + 8.0), Vector2(32.0, 24.0),
		"res://scenes/overworld/maps/PalletTown.tscn", Vector2(160.0, 208.0))
