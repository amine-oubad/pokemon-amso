extends Node2D
## ARÈNE DE PARMANIE — type Poison. Champion : Koga.
## Layout 320×240 : salle ninja avec murs invisibles.
...
func _build_ground() -> void:
	_rect(Vector2.ZERO, Vector2(MAP_W, MAP_H), Color(0.15, 0.08, 0.20))
	_rect(Vector2(144, 0), Vector2(32, MAP_H), Color(0.20, 0.12, 0.25))

func _build_arena() -> void:
	# Zones empoisonnées (violet/vert)
	_rect(Vector2(16, 80), Vector2(112, 80), Color(0.25, 0.12, 0.35))
	_rect(Vector2(192, 80), Vector2(112, 80), Color(0.25, 0.12, 0.35))
	_wall(Vector2(72, 120), Vector2(112, 80))
	_wall(Vector2(248, 120), Vector2(112, 80))
	# Poison pools
	_rect(Vector2(40, 100), Vector2(24, 24), Color(0.40, 0.15, 0.55, 0.6))
	_rect(Vector2(240, 100), Vector2(24, 24), Color(0.40, 0.15, 0.55, 0.6))

	_rect(Vector2(100, 16), Vector2(120, 56), Color(0.30, 0.15, 0.40))
	_rect(Vector2(100, 16), Vector2(120, 8), Color(0.22, 0.10, 0.30))

	var title := Label.new()
	title.text     = "ARÈNE DE PARMANIE"
	title.position = Vector2(104, 4)
	title.add_theme_font_size_override("font_size", 7)
	title.add_theme_color_override("font_color", Color(0.70, 0.30, 0.90))
	add_child(title)
...
func _build_trainers() -> void:
	_trainer(Vector2(144.0, 160.0), "fuchsia_trainer_1", Color(0.40, 0.20, 0.55))
	_trainer(Vector2(176.0, 112.0), "fuchsia_trainer_2", Color(0.35, 0.18, 0.50))
	_trainer(Vector2(160.0, 48.0),  "gym_leader_koga",   Color(0.55, 0.20, 0.70))

func _build_transitions() -> void:
	_transition(Vector2(160.0, MAP_H + 8.0), Vector2(32.0, 24.0),
		"res://scenes/overworld/maps/FuchsiaCity.tscn", Vector2(256.0, 76.0))
...
func _on_trainer_battle(_trainer_id: String) -> void:
	GameState.return_to_scene = "res://scenes/overworld/maps/FuchsiaGym.tscn"
