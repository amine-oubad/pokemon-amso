extends Node2D
## ARÈNE DE CARMIN — type Électrique. Champion : Major Bob.
## Layout 320×240 : salle avec éclairs décoratifs.
...
func _build_ground() -> void:
	_rect(Vector2.ZERO, Vector2(MAP_W, MAP_H), Color(0.20, 0.18, 0.12))
	_rect(Vector2(144, 0), Vector2(32, MAP_H), Color(0.25, 0.22, 0.15))

func _build_arena() -> void:
	# Panneaux électriques décoratifs
	_rect(Vector2(16, 80), Vector2(112, 80), Color(0.75, 0.65, 0.10))
	_rect(Vector2(192, 80), Vector2(112, 80), Color(0.75, 0.65, 0.10))
	_wall(Vector2(72, 120), Vector2(112, 80))
	_wall(Vector2(248, 120), Vector2(112, 80))

	_rect(Vector2(100, 16), Vector2(120, 56), Color(0.80, 0.70, 0.15))
	_rect(Vector2(100, 16), Vector2(120, 8), Color(0.65, 0.55, 0.10))

	var title := Label.new()
	title.text     = "ARÈNE DE CARMIN"
...
func _build_trainers() -> void:
	_npc(Vector2(120.0, 192.0), "sign_carmin_gym", "", "", Color(0.80, 0.70, 0.20))
	_trainer(Vector2(144.0, 160.0), "carmin_trainer_1", Color(0.75, 0.65, 0.15))
	_trainer(Vector2(176.0, 112.0), "carmin_trainer_2", Color(0.70, 0.60, 0.10))
	_trainer(Vector2(160.0, 48.0), "gym_leader_bob",    Color(0.90, 0.80, 0.10))

func _build_transitions() -> void:
	_transition(Vector2(160.0, MAP_H + 8.0), Vector2(32.0, 24.0),
		"res://scenes/overworld/maps/CarminCity.tscn", Vector2(72.0, 84.0))
...
func _on_trainer_battle(_trainer_id: String) -> void:
	GameState.return_to_scene = "res://scenes/overworld/maps/CarminGym.tscn"
