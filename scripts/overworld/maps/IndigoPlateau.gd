extends Node2D
## PLATEAU INDIGO — lobby de la Ligue Pokémon.
## Layout 320×240 : Centre Pokémon + entrée de la Ligue.
...
func _build_ground() -> void:
	_rect(Vector2.ZERO, Vector2(MAP_W, MAP_H), Color(0.22, 0.22, 0.30))

func _build_path() -> void:
	_rect(Vector2(0, 104), Vector2(MAP_W, 32), Color(0.45, 0.40, 0.50))
	_rect(Vector2(144, 0), Vector2(32, 104), Color(0.45, 0.40, 0.50))

func _build_buildings() -> void:
	_building(Vector2(16.0, 136.0), Vector2(80.0, 48.0), Color(0.85, 0.20, 0.18), "CENTRE\nPOKÉMON")
	_building(Vector2(100.0, 8.0), Vector2(120.0, 64.0), Color(0.70, 0.65, 0.20), "LIGUE\nPOKÉMON")
...
func _build_npcs() -> void:
	_npc(Vector2(56.0, 184.0), "indigo_npc1", "heal_team", "", Color(0.90, 0.70, 0.70))
	_npc(Vector2(200.0, 180.0), "indigo_npc2", "",  "",        Color(0.60, 0.60, 0.70))
	# Garde à l'entrée de la Ligue
	_npc(Vector2(160.0, 80.0), "", "start_league", "",         Color(0.80, 0.75, 0.20))

func _build_signs() -> void:
	_sign(Vector2(100.0, 80.0), "indigo_sign1")

func _build_transitions() -> void:
	# Ouest → Route 8 (retour)
