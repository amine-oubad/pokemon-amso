extends Node2D
## ROUTE 8 — Route entre Parmanie et Cramois'Île.
## Layout 480×240 avec deux zones d'herbe (eau/feu).
...
func _build_ground() -> void:
	_rect(Vector2.ZERO, Vector2(MAP_W, MAP_H), Color(0.30, 0.25, 0.18))

func _build_path() -> void:
	_rect(Vector2(0, 104), Vector2(MAP_W, 32), Color(0.55, 0.45, 0.32))

func _build_grass() -> void:
	# Zone eau (bleu-vert)
	_rect(Vector2(48, 0), Vector2(160, 104), Color(0.15, 0.28, 0.35))
	_rect(Vector2(48, 136), Vector2(160, 104), Color(0.15, 0.28, 0.35))
	# Zone feu (orange-rouge)
	_rect(Vector2(270, 0), Vector2(160, 104), Color(0.35, 0.20, 0.12))
	_rect(Vector2(270, 136), Vector2(160, 104), Color(0.35, 0.20, 0.12))
...
func _build_trainers() -> void:
	_trainer(Vector2(160.0, 120.0), "route8_hiker",   Color(0.55, 0.40, 0.30))
	_trainer(Vector2(340.0, 120.0), "route8_swimmer", Color(0.30, 0.50, 0.70))

func _build_signs() -> void:
	_sign(Vector2(48.0, 120.0), "route8_sign1")

func _build_transitions() -> void:
	# Ouest → Parmanie
	_transition(Vector2(-8.0, 120.0), Vector2(24.0, 32.0),
		"res://scenes/overworld/maps/FuchsiaCity.tscn", Vector2(304.0, 120.0))
	# Est → Cramois'Île
	_transition(Vector2(MAP_W + 8.0, 120.0), Vector2(24.0, 32.0),
		"res://scenes/overworld/maps/CramoisIle.tscn", Vector2(16.0, 120.0))
...
func _build_encounter_zones() -> void:
	_encounter_zone(Vector2(128.0, 120.0), Vector2(160.0, 240.0), "route8", "grass_01", 0.15)
	_encounter_zone(Vector2(350.0, 120.0), Vector2(160.0, 240.0), "route8", "grass_02", 0.15)
...
func _on_battle_started(enemy_data: Dictionary, is_trainer: bool) -> void:
	...
	GameState.return_to_scene  = "res://scenes/overworld/maps/Route8.tscn"
