extends Node2D
## ARÈNE DE SAFRANIA — type Psy. Championne : Morgane.
## Layout 320×240 : salle mystique.
...
func _build_ground() -> void:
	_rect(Vector2.ZERO, Vector2(MAP_W, MAP_H), Color(0.18, 0.10, 0.28))
	_rect(Vector2(144, 0), Vector2(32, MAP_H), Color(0.22, 0.14, 0.32))

func _build_arena() -> void:
	# Cercles mystiques (représentés par des carrés stylisés)
	_rect(Vector2(16, 80), Vector2(112, 80), Color(0.30, 0.15, 0.45))
	_rect(Vector2(192, 80), Vector2(112, 80), Color(0.30, 0.15, 0.45))
	_wall(Vector2(72, 120), Vector2(112, 80))
	_wall(Vector2(248, 120), Vector2(112, 80))
	# Symboles psy
	_rect(Vector2(50, 105), Vector2(16, 16), Color(0.70, 0.40, 0.90, 0.6))
	_rect(Vector2(230, 105), Vector2(16, 16), Color(0.70, 0.40, 0.90, 0.6))

	_rect(Vector2(100, 16), Vector2(120, 56), Color(0.35, 0.18, 0.50))
	_rect(Vector2(100, 16), Vector2(120, 8), Color(0.25, 0.12, 0.38))

	var title := Label.new()
	title.text     = "ARÈNE DE SAFRANIA"
...
func _build_trainers() -> void:
	_npc(Vector2(120.0, 192.0), "sign_safrania_gym", "", "", Color(0.55, 0.35, 0.70))
	_trainer(Vector2(144.0, 160.0), "safrania_trainer_1", Color(0.50, 0.30, 0.65))
	_trainer(Vector2(176.0, 112.0), "safrania_trainer_2", Color(0.45, 0.25, 0.60))
	_trainer(Vector2(160.0, 48.0), "gym_leader_morgane",  Color(0.70, 0.40, 0.90))

func _build_transitions() -> void:
	_transition(Vector2(160.0, MAP_H + 8.0), Vector2(32.0, 24.0),
		"res://scenes/overworld/maps/SafraniaCity.tscn", Vector2(256.0, 76.0))
...
func _on_trainer_battle(_trainer_id: String) -> void:
	GameState.return_to_scene = "res://scenes/overworld/maps/SafraniaGym.tscn"
